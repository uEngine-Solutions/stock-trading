...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        Ask # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
        Ask
    </v-card-title >
    ...
    <v-card-text style = "margin-left:-15px; margin-top:10px;">
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="AccountId" v-model="value.accountId"/>
          </div>
          <div class="grey--text ml-4" v-else>
    ...
    methods: {
      ...
      async save(){
        try{
          ...
          if(this.isNew){
            temp = await axios.post(axios.fixUrl('/asks'), this.value)
          }else{
